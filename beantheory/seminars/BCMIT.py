    ...
    def html_talks(self):
        ...
        for row in self.table:
            ...
            try:
                ...
                self.errors.append(
                    "Could not parse (day, place, room): {} ".format(row[0])
                )
                continue
            try:
            ...
            for talk_row in row[1:]:
                ...
                try:
                    start_h, start_m, end_h, end_m, rest = hours_minutes_rest.findall(
                        talk_row
                    )[0]
                except Exception:
                    self.errors.append(
                        "Could not parse (start_h, start_m, end_h, end_m, rest): {} ".format(
                            talk_row
                        )
                    )
                    continue
                try:
                    ...
                    self.errors.append(
                        "Could not parse (speaker, title): {} ".format(rest)
                    )
                    continue
                talk["time"] = day + timedelta(
                    hours=12 + int(start_h), minutes=int(start_m)
                )
                talk["endtime"] = day + timedelta(
                    hours=12 + int(end_h), minutes=int(end_m)
                )
                talk["place"] = place
                talk["room"] = room
                talk["note"] = "BC-MIT"
